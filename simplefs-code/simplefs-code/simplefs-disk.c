...
void simplefs_readDataBlock(int blocknum, char *buf){
   ...
   if (blockNum < 0 || blockNum >= TOTAL_BLOCKS) {
    ...
    lseek(diskFD, blockNum * BLOCKSIZE, SEEK_SET);
    int ret = read(diskFD, buffer, BLOCKSIZE);
    
    // Check if the read was successful
    if (ret != BLOCKSIZE) {
        printf("Error: Failed to read block %d, read %d bytes instead of %d\n", blockNum, ret, BLOCKSIZE);
        memset(buffer, 0, BLOCKSIZE); // Initialize buffer to avoid garbage data
        return -1;
    }
...
void simplefs_writeDataBlock(int blocknum, char *buf){
	...
	*/
    ...
    lseek(diskFD, blockNum * BLOCKSIZE, SEEK_SET);
    int ret = write(diskFD, buffer, BLOCKSIZE);
    
    if (ret != BLOCKSIZE) {
        printf("Error: Failed to write block %d, wrote %d bytes instead of %d\n", blockNum, ret, BLOCKSIZE);
        return -1;
    }
